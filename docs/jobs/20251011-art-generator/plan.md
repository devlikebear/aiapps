...
**작업 항목**:
- [ ] `app/api/art/edit/route.ts` 구현
  - 프롬프트 기반 이미지 편집
  - 영역 선택 편집 (마스크)
  - 객체 추가/제거
- [ ] `components/art/ImageEditor.tsx` 구현
  - 이미지 선택 UI
  - 편집 프롬프트 입력
  - 편집 영역 선택 도구
  - Before/After 비교
- [ ] 갤러리에서 편집 버튼 추가
- [ ] 편집 히스토리 저장 (원본 이미지 보존)

**참고 파일**:
- `app/api/art/generate/route.ts` - API 구조 참고
- `components/art/ReferenceImageUploader.tsx` - 이미지 업로드 로직
...
**목표**: 한 이미지의 스타일을 다른 이미지에 적용

**작업 항목**:
- [ ] `app/api/art/style-transfer/route.ts` 구현
  - 베이스 이미지 입력
  - 스타일 이미지 입력
  - 스타일 강도 조절
- [ ] `components/art/StyleTransfer.tsx` 구현
  - 베이스/스타일 이미지 선택
  - 강도 슬라이더
  - 실시간 미리보기
- [ ] 프리셋 스타일 갤러리
...
1. **우선순위 결정**: Phase 1의 어떤 기능부터 시작할지 결정
2. **GitHub Issue 생성**: 각 작업 항목별로 Issue 생성
3. **Feature 브랜치 생성**: `feature/[issue-number]-[description]`
4. **개발 시작**: TDD 방식으로 테스트 먼저 작성

**추천 시작점**:
- Phase 1.1 (이미지 편집) - Gemini API 활용 확장
- Phase 1.4 (갤러리 검색) - 즉시 체감 가능한 개선
