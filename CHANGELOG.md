...
### Added
- ✨ **톤 시스템 고도화**
  - ToneInfo 인터페이스로 각 톤의 상세 정보 구조화
  - 톤별 명확한 설명 및 AI 프롬프트 영향 가이드
  - 톤 선택 시 실시간 상세 정보 표시
  - 각 톤의 사용 예시 제공
...
- 🎯 **커스텀 톤 기능**
  - 프리셋 생성 시 커스텀 톤 설명 입력 가능
  - 커스텀 톤 체크박스 토글 UI
  - 프리셋 선택 시 커스텀 톤 자동 적용
  - AI 프롬프트에 커스텀 톤 정보 포함

- ✨ **트윗 생성기 프리셋 자동 적용**
  - 프리셋 선택 후 생성 페이지에서 자동으로 설정 반영
  - 쿼리 파라미터를 통한 프리셋 ID 전달
  - `getPreset(id)` 함수 추가로 프리셋 조회 개선
  - 페이지 헤더에 현재 사용 중인 프리셋 이름 표시

### Improved
- 톤 설명의 명확성과 상세함 대폭 개선
- 프롬프트 입력 필드에 현재 톤 정보 표시
- 톤 선택 UI 개선 (버튼 방식)
- 트윗 생성기 프리셋 사용성 개선
- 네비게이션 구조 개선

### Fixed
- 트윗 프리셋이 생성기 페이지에 반영되지 않는 문제 (#108)
- 모든 생성기에서 GNB를 통한 랜딩페이지 접근 불가 문제 (#109)
...
### Fixed
- 오디오 생성기 최근 아티팩트 리스트 UI 누락 문제 수정
- 아트 생성기 최근 아티팩트 리스트 UI 누락 문제 수정
- 모든 생성기에서 최근 생성 목록이 정상 표시
