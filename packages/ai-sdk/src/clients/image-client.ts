...
export class GeminiImageClient {
  ...
  constructor(config: ImageClientConfig) {
    this.config = { ...DEFAULT_CONFIG, ...config } as Required<ImageClientConfig>;
  ...
  async generate(request: ImageGenerateRequest): Promise<ImageGenerateResponse> {
    ...
    logger.info('Starting image generation', { requestId, prompt: request.prompt });
   ...
   * 스타일 전이
   */
  async styleTransfer(request: ImageStyleTransferRequest): Promise<ImageStyleTransferResponse> {
    ...
    return withRetry(async () => {
      ...
      const prompt = request.prompt || 'Apply the style from the second image to the first image';
   ...
   * 이미지 데이터 준비
   */
  private async prepareImagePart(image: any): Promise<any> {
  ...
  private validateGenerateRequest(request: ImageGenerateRequest): void {
    ...
    if (request.numberOfImages && (request.numberOfImages < 1 || request.numberOfImages > 4)) {
  ...
  private validateComposeRequest(request: ImageComposeRequest): void {
    if (!request.images || request.images.length < 2 || request.images.length > 3) {
   ...
   * 스타일 전이 요청 검증
   */
  private validateStyleTransferRequest(request: ImageStyleTransferRequest): void {
